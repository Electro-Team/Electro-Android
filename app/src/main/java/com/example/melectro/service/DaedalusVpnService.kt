...
import com.example.melectro.provider.Provider
import com.example.melectro.provider.ProviderPicker
import com.example.melectro.receiver.StatusBarBroadcastReceiver
...
class DaedalusVpnService : VpnService(), Runnable {
    ...
    override fun onCreate() {
        ...
        if (Daedalus.getPrefs()!!.getBoolean("settings_use_system_dns", false)) {
            ...
            }.also { receiver = it }, IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION))
        }
    }
            ...
            ACTION_ACTIVATE -> {
                ...
                if (Daedalus.getPrefs()!!.getBoolean("settings_notification", true)) {
                    ...
                    builder.setWhen(0)
                        ...
                        .setColor(resources.getColor(R.color.colorPrimary)) //backward compatibility
                        .setAutoCancel(false)
                        .setOngoing(true)
    ...
    override fun run() {
        try {
            ...
            if (Daedalus.getPrefs()!!.getBoolean("settings_app_filter_switch", false)) {
                val apps: ArrayList<String> = Daedalus.configurations?.appObjects!!
                if (apps.size > 0) {
                    val mode: Boolean =
                        Daedalus.getPrefs()!!.getBoolean("settings_app_filter_mode_switch", false)
                    for (app in apps) {
                        try {
                            if (mode) {
            ...
            val advanced: Boolean =
                Daedalus.getPrefs()!!.getBoolean("settings_advanced_switch", false)
            ...
            MainActivity.instance?.runOnUiThread {
                AlertDialog.Builder(MainActivity.instance!!)
                    .setTitle(com.example.melectro.R.string.error_occurred)
